...
export const checkPassword = async (plain: string, encryptedPassword: string): Promise<boolean> => {
  const passwords = encryptedPassword.split('$');

  const encrypted = passwords.slice().pop();
  const salt = passwords[1];

  const combined = await Array.from({ length: 512 }, (_, i) => i).reduce(async (_acc, curr) => {
...
export const generatePassword = async (plain: string): Promise<string> => {
  ...
  const encrypted = await Array.from({ length: 512 }, (_, i) => i).reduce(async (_acc, curr) => {
    const acc = await _acc;
    if (curr === 0) {
      return await md5(`${salt}${plain}${curr}`);
    }
